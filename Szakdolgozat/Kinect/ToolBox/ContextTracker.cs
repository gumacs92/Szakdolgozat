...
namespace Szakdolgozat.Kinect.ToolBox
{
    public class ContextTracker
    {
        ...
        public void Add(Skeleton skeleton, JointType jointType)
        {
            ...
            var position = Vector.ToVector(skeleton.Joints.Where(j => j.JointType == jointType).First().Position);
        ...
        public bool IsStable(int trackingID)
        {
            ...
            if (currentPoints.Count != windowSize)
                return false;
            Vector current = currentPoints[currentPoints.Count - 1].Position;
        ...
        public bool IsStableRelativeToCurrentSpeed(int trackingID)
        {
            ...
            if (currentPoints.Count < 2)
                return false;
            Vector previousPosition = currentPoints[currentPoints.Count - 2].Position;
            Vector currentPosition = currentPoints[currentPoints.Count - 1].Position;
        ...
        public bool IsStableRelativeToAverageSpeed(int trackingID)
        {
            ...
            if (currentPoints.Count != windowSize)
                return false;
            Vector startPosition = currentPoints[0].Position;
            Vector currentPosition = currentPoints[currentPoints.Count - 1].Position;
        ...
        {
            var leftShoulderPosition = Vector.ToVector(skeleton.Joints.Where(j => j.JointType == JointType.ShoulderLeft).First().Position);
            var rightShoulderPosition = Vector.ToVector(skeleton.Joints.Where(j => j.JointType == JointType.ShoulderRight).First().Position);
